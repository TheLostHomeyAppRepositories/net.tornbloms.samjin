...
class IM6001Device extends ZigBeeDevice {
   ...
   async onNodeInit({ zclNode }: { zclNode: any }) {
      ...
      this.log('Samjin Multi Sensor device initialized');
      ...
      this.registerAttrReportListener(
         ...
         this.error(
            'Failed to register attribute report listener for batteryPercentageRemaining:',
            err
         );
      });
      ...
      this.registerAttrReportListener(
         ...
         // Log an error if the listener registration fails
         this.error(
            'Failed to register attribute report listener for measuredValue:',
            err
         );
      });
      ...
      this.registerAttrReportListener(
         ...
         // Log an error if the listener registration fails
         this.error(
            'Failed to register attribute report listener for zoneStatus:',
            err
         );
      });
   ...
   async onSettings({
      ...
      this.log('IM6001Device settings were changed');
   }
